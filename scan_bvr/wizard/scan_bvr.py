...
class scan_bvr(osv.osv_memory):
    ...
    def _get_invoice_address(self, cr, uid, partner_address):
                ...
                if not valid_address_id:
                        if len(partner_address) > 0:
                            ...
                            raise osv.except_osv('AddressError',
    ...
    def _create_direct_invoice(self, cr, uid, data, context):
                    ...
                    # #
                    #
                    p = self.pool.get('res.partner').browse(cr, uid, account_info.partner_id.id)
    ...
    def _get_bvr_structurated(self, bvr_string):
        if bvr_string != False:
            ...
            #
                ...
                raise osv.except_osv('BVR Type error',
    ...
    def validate_bvr_string(self, cr, uid, ids, context):
        ...
        if partner_bank_search:
            ...
            # bvr_adhreent_number so we can directly create the account
            ...
            self.write(cr, uid, ids, {'state': 'need_extra_info'}, context)
            return {
                'name': _("Scan BVR"),
                'view_mode': 'form',
                'view_type': 'form',
                ...
                'nodestroy': True,
                'target': 'new',
                'context': {'default_state': 'need_extra_info',
                            'default_bvr_string': bvr_data.bvr_string,
                            'default_journal_id': bvr_data.journal_id.id}
            }
